...
//подсмотрел в дискорде сам до конца не особо понимаю как тут все работает

function checkMaxStringLength(verifiedString, minLength, maxLength) {

  const userInput = verifiedString.value;
  if (verifiedString > 0 && userInput.length >= minLength && userInput.length <= maxLength) {
    return true;
  } else {
    return false;
  }
...
checkMaxStringLength (20,20,140);
